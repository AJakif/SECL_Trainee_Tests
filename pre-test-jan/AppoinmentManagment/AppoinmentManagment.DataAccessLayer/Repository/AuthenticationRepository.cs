...
namespace AppoinmentManagment.DataAccessLayer.Repository
{
    public class AuthenticationRepository : IAuthenticationRepository
    {
        ...
        public AuthenticationRepository(IConfiguration config, ILogger<AuthenticationRepository> logger)
        {
            _config = config;
            _logger = logger;
        }
        ...
        public UserModel LoginByEmail(LoginBO lbo)
        {
                ...
                using (SqlDataReader dataReader = command.ExecuteReader())
                {
                    try
                    {
                        while (dataReader.Read()) //make it single user
                        {
                            ...
                            user.TypeId= Convert.ToInt32(dataReader["TypeId"]);
